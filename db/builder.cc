...
Status BuildLevel0Table(const std::string& dbname, Env* env, const Options& options,
                  TableCache* table_cache, Iterator* iter, FileMetaData* meta,int level) {

  std::cout<<"buildlevel0table"<<std::endl;
  Status s;
  meta->file_size = 0;
  iter->SeekToFirst();
  ...
  if (iter->Valid()) {
    ...
    TableBuilder* builder = new TableBuilder(options, file, meta->key, level);
    meta->smallest.DecodeFrom(iter->key());
    for (; iter->Valid(); iter->Next()) {
      Slice key = iter->key();
      meta->largest.DecodeFrom(key);
